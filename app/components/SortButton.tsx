        ...
        <Listbox value={value} onChange={handleOnChangeClick}>
            <>
                <div className="relative selectorContainor_Sort">
                    <Listbox.Button
                        className="relative w-full bg-white py-1 pl-3 pr-10 text-left text-gray-900 listboxButton_Sort "
                        style={{height: '50px'}}>
            <span className="flex items-center">
              ...
              <PlayIcon className="h-3 w-3 mr-2 text-white text-gray-900 transform rotate-90" aria-hidden="true"/>
                    ...
                    <Transition>
                        <Listbox.Options
                            className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm options_Sort">
                            {sort.map((person) => (
                                <Listbox.Option
                                    key={person.id}
                                    className={({active}) =>
